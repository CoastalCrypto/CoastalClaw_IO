...
## Chunk 0: Workspace plumbing (foundation)

**Why first:** Plan 1 spreads code across `packages/core` (types, store, HTTP routes) and `packages/architect` (daemon, stages, adapters). The architect package needs to import from core. The existing setup (verified) doesn't yet have that dependency declared, and core has no `exports` map for subpath imports under Node16 module resolution. Fix this once before anything else.

**Files:**
- Modify: `packages/architect/package.json` — add `"@coastal-ai/core": "workspace:*"` to dependencies.
- Modify: `packages/core/package.json` — add an `exports` map declaring the subpaths Plan 1 will use.

- [ ] **Step 1: Add the workspace dependency**

Edit `packages/architect/package.json`. Inside `"dependencies"`, add `"@coastal-ai/core": "workspace:*"` so the block becomes:
...
- [ ] **Step 2: Add the subpath exports map to core**

Edit `packages/core/package.json`. Below `"types"` and above `"bin"`, add the `"exports"` field:
...
"exports": {
  ...
  "./architect/types": {
    "types": "./dist/architect/types.d.ts",
    "default": "./dist/architect/types.js"
  },
  "./architect/db": {
    "types": "./dist/architect/db.d.ts",
    "default": "./dist/architect/db.js"
  },
  "./architect/store": {
    "types": "./dist/architect/store.d.ts",
    "default": "./dist/architect/store.js"
  },
  "./architect/cycle-store": {
    "types": "./dist/architect/cycle-store.d.ts",
    "default": "./dist/architect/cycle-store.js"
  },
  "./architect/dedup": {
    "types": "./dist/architect/dedup.d.ts",
    "default": "./dist/architect/dedup.js"
  },
  "./architect/ulid": {
    "types": "./dist/architect/ulid.d.ts",
    "default": "./dist/architect/ulid.js"
  }
},
```

(Comma-separated and inserted before `"bin"`. Mirrors the daemon's pattern — daemon already declares `@coastal-ai/core: workspace:*`. Module resolution: `Node16`, so dual `types`/`default` entries are required for both TypeScript and Node to find the right files.)

- [ ] **Step 3: Install**

Run: `pnpm install`
Expected: lockfile updates with the new architect→core link; no errors. The `packages/architect/node_modules/@coastal-ai/core` symlink should exist.

- [ ] **Step 4: Add a pretest hook so architect's tests always see a built core**

The `exports` map points to `./dist/architect/*.js` — files that don't exist until `pnpm --filter core build` has run. Without a guard, running `pnpm --filter architect test` immediately after a fresh checkout fails with `ERR_MODULE_NOT_FOUND` (or worse, silently picks up stale dist files from a prior build).

Edit `packages/architect/package.json` and add a `pretest` script that builds core first:

```json
"scripts": {
  "build": "tsc",
  "dev": "node --loader ts-node/esm src/index.ts",
  "pretest": "pnpm --filter @coastal-ai/core build",
  "test": "vitest run"
}
```

This is **not optional**. Subsequent chunks all assume `@coastal-ai/core/architect/*` resolves at test time. Skipping the pretest hook works locally as long as someone has built core recently, but breaks fresh checkouts and CI.

- [ ] **Step 5: Verify import resolution end-to-end**

Add a one-line throwaway test to confirm imports work before any real code lands:

Create `packages/architect/src/__tests__/workspace-link.test.ts`:
...
Run: `pnpm --filter architect test src/__tests__/workspace-link.test.ts`
Expected: PASS. The pretest hook runs `pnpm --filter @coastal-ai/core build` automatically; if THAT fails, the test infrastructure can't be trusted yet — fix the core build before continuing.

> If the architect test still fails with `ERR_MODULE_NOT_FOUND` after a successful core build: verify the `exports` map keys in `packages/core/package.json` match what `@coastal-ai/core/architect/types` resolves to (subpath `./architect/types` → `./dist/architect/types.js`). Node 16 ESM resolution requires the subpath to match an exports key exactly.

- [ ] **Step 6: Commit**

```bash
git add packages/architect/package.json packages/core/package.json pnpm-lock.yaml \
        packages/architect/src/__tests__/workspace-link.test.ts
git commit -m "chore(architect): wire up @coastal-ai/core dependency + subpath exports

- packages/architect/package.json now declares
  @coastal-ai/core: workspace:* (matching daemon's pattern).
- packages/architect pretest hook builds @coastal-ai/core first
  so subpath imports always see a populated dist/.
- packages/core/package.json gains an exports map covering the
  architect subpaths Plan 1 will populate (types, db, store,
  cycle-store, dedup, ulid).
- Throwaway workspace-link.test.ts confirms imports resolve
  through the symlink + exports map.

Plan 1 / Chunk 0 — workspace plumbing.

Co-Authored-By: Claude Opus 4.7 (1M context) <noreply@anthropic.com>"
```

**Import convention for the rest of Plan 1:**

Cross-package imports inside `packages/architect/src/**` MUST use the bare specifier with the subpath:

```ts
import { WORK_ITEM_STATUSES } from '@coastal-ai/core/architect/types'
import { WorkItemStore } from '@coastal-ai/core/architect/store'
import { CycleStore } from '@coastal-ai/core/architect/cycle-store'
```

Inside `packages/core/src/**`, use relative imports as usual:

```ts
// from packages/core/src/api/routes/architect.ts:
import { WorkItemStore } from '../../architect/store.js'
```

Every import in subsequent chunks follows this convention. Do not use `'../../core/architect/...'` from architect — that path resolves to `packages/architect/core/...` which does not exist.
...
- [ ] **Step 1: Inspect existing admin-route shape (research-first, mandatory)**

Open `packages/core/src/api/routes/skills.ts`, `packages/core/src/api/routes/agents.ts`, and `packages/core/src/api/routes/admin.ts`. Confirm:

1. **Where admin auth lives.** Look for: a Fastify decorator (`app.decorate('verifyAdmin', ...)`), a `preHandler` hook, a wrapper function, or a route-prefix plugin. The plan code below assumes `app.verifyAdmin` exists as a decorator — *verify this first*.
2. **The exact signature.** Is it `async (req, reply) => {}` returning void on success and replying on failure? Or does it throw? The plan's `preHandler: app.verifyAdmin` syntax requires a void-returning handler.
3. **Headers.** Existing admin routes likely read `x-admin-session` or `x-admin-token` — confirm which.

If the actual implementation differs, **adjust the route definition below** before writing the test. Do not write code against an assumed API. Common variations and their fix:

| Reality | Adjustment |
|---|---|
| `async function verifyAdmin(req, reply) { ... }` exported as a function (not a decorator) | Replace `preHandler: (app as any).verifyAdmin` with `preHandler: verifyAdmin` and import the function. |
| Auth via plugin that wraps a sub-router (e.g. `await app.register(adminPlugin, { prefix: '/api/admin' })`) | Drop the `preHandler` from each route; register `architect.ts` inside the same admin sub-router. |
| Header is `x-admin-token` not `x-admin-session` | Update the smoke-test curl in Step 7. |

Capture the verified pattern in a comment at the top of the new `architect.ts` so future chunks don't re-derive it.
...
- [ ] **Step 1: Inspect existing ModelRouter (research-first, mandatory)**

The plan code below assumes a `ModelRouterLike` interface with `getModelFor(domain, priority)` and `generate(modelId, prompt)` methods, plus a tier system (`lite | standard | apex`) that aligns with the existing Hardware-Aware Tiers. **Verify before writing code.**

```bash
# Find the actual class
grep -rn "class ModelRouter\|registerDomain\|getModelFor\|class TinyRouter\|class CascadeRouter" \
  packages/core/src/ | head -20

# Read the closest match end-to-end
head -120 packages/core/src/models/ModelRouter.ts 2>/dev/null \
  || head -120 packages/core/src/routing/ModelRouter.ts 2>/dev/null \
  || head -120 packages/core/src/router.ts 2>/dev/null
```

Three things to confirm before writing the wrapper:

1. **Method names.** Is it `getModelFor(domain, priority)` or `route(domain, priority)` or `selectModel({domain, priority})`?
2. **Tier shape.** Does the model descriptor expose `tier: 'lite'|'standard'|'apex'`, or `tier: 'small'|'medium'|'large'`, or no tier at all? If no tier, infer it from VRAM in the existing `Hardware-Aware Tiers` system (per spec §3 minimum-capability gate, which references the README's Lite/Standard/Apex naming).
3. **Generate API.** Is the call `await router.generate(modelId, prompt)` or `await client.complete({model, prompt})` or `await router.invoke(...)`?

If any of these differ, rename the `ModelRouterLike` interface fields below and adjust call sites in Step 4 to match. The wrapper's *purpose* (route plan calls high, summary calls low; gate startup on min-tier) is unchanged regardless.

Capture the verified API in a comment at the top of `model-router-client.ts`.
...
Using the API surface confirmed in Step 1 (method names + tier shape), register the new `architect` domain in the existing ModelRouter setup. The exact call shape depends on what Step 1 found:

| Step 1 finding | Step 6 call |
|---|---|
| `router.registerDomain(name, opts)` exists | `router.registerDomain('architect', { tiers: { high: 'best-available', low: 'fast' } })` |
| Domains live in a config file (e.g. `data/.coastal-models.json`) | Add `"architect"` entry to that file with the tier mapping |
| Domains are registered via constructor option | Extend the constructor invocation in core's bootstrap |
| **No registration mechanism exists** | The `ArchitectModelRouterClient` falls back to whatever the router default-routes; skip Step 6 entirely and note in commit body |

Add a one-line comment at the registration site: `// architect domain — see packages/architect/src/model-router-client.ts`.

If Step 6 produced no file change (last row of the table), the commit in Step 7 stages only the new architect-side files. If it did produce a change, stage that file as well.

- [ ] **Step 7: Commit**

Determine the exact `git add` line based on Step 6's outcome — do not blindly stage `ModelRouter.ts` if it wasn't modified. Two valid forms:

```bash
# If Step 6 produced no core-side file change:
git add packages/architect/src/model-router-client.ts \
        packages/architect/src/__tests__/model-router-client.test.ts

# If Step 6 modified a core-side file (replace <path> with the actual file from Step 6):
git add packages/architect/src/model-router-client.ts \
        packages/architect/src/__tests__/model-router-client.test.ts \
        <path-from-step-6>
```
...
(No `loadWorkspaceMap` stub here. The real implementation lives in `workspace-map.ts` and is added in Step 5 of this chunk. Keeping a throwing stub here would be a tripwire for any caller that imports it before Step 5 runs.)
...
describe('loadWorkspaceMapSync', () => {
  it('finds the actual Coastal.AI packages', () => {
    // From packages/architect/src/__tests__/, four '..' segments reach the repo root:
    //   __tests__ → src → architect → packages → REPO_ROOT
...
- [ ] **Step 1: Rewrite existing config.test.ts assertions, then add new tests**

The existing `isLockedPath` returns `boolean` (verified). Step 3 changes the return type to `string | null`. **Every existing assertion in `config.test.ts` must be converted accordingly before the new tests are added.** Concrete sweep:

| Old | New |
|---|---|
| `expect(isLockedPath(p)).toBe(true)` | `expect(isLockedPath(p)).toBeTruthy()` |
| `expect(isLockedPath(p)).toBe(false)` | `expect(isLockedPath(p)).toBeNull()` |
| `expect(isLockedPath(p)).toEqual(true)` | `expect(isLockedPath(p)).toBeTruthy()` |
| `expect(isLockedPath(p)).toEqual(false)` | `expect(isLockedPath(p)).toBeNull()` |
| `expect(LOCKED_PATHS.has(...)).toBe(true)` (or `.size === N`) | DELETE — old `Set<string>` is gone; behaviour tests subsume |
| Any assertion that `packages/architect/...` is allowed | DELETE — self-modify guard now blocks the whole tree |

After the sweep, append the new behaviour tests below to the existing describe blocks:
...
> **Critical:** preserve the existing `posix.normalize` + backslash-to-forward-slash + lowercase + `./` strip preamble. Existing callers (and tests) depend on it. Also: keep the existing path-literal allowlist (`packages/core/src/agents/permission-gate.ts`, `agents/action-log.ts`, `api/routes/admin.ts`) — these are security-critical and must NOT be droppable to `packages/architect/**` regex coverage alone.

```ts
// packages/architect/src/config.ts (replaces existing implementation)
import { posix } from 'node:path'
import { existsSync, readFileSync } from 'node:fs'
import { join } from 'node:path'

/**
 * Path patterns the architect is never allowed to modify.
 * These are matched against NORMALIZED paths (forward-slashed, lowercased,
 * leading ./ stripped) — see normalizeForCheck below.
 */
const DEFAULT_LOCKED_PATTERNS: RegExp[] = [
  ...
  /^packaging\//,
  /^coastalos\/build\//,
  /^packages\/architect\//,                                          // self-modify guard (broad)
  /^packages\/core\/src\/agents\/permission-gate\.ts$/,              // security: trust enforcement
  /^packages\/core\/src\/agents\/action-log\.ts$/,                   // security: audit log
  /^packages\/core\/src\/api\/routes\/admin\.ts$/,                   // security: admin auth surface
]

/** Normalize a path for locked-path matching: forward slashes, lowercased, leading ./ stripped, posix.normalize. */
function normalizeForCheck(relPath: string): string {
  return posix.normalize(
    relPath
      ...
      .toLowerCase()               // case-insensitive on Windows NTFS / macOS HFS+
  )
}

/**
 * Returns null if the path is allowed, or a descriptive string if it matches
 * a locked pattern. (Return-type change from existing boolean — see Step 1.)
 */
...
/** Load operator-supplied regex overrides from data/.architect-locked-paths (one regex per line). */
...
The old `LOCKED_PATHS` `Set<string>` and the old boolean `isLockedPath` are deleted. The 7 path literals from the old set are now covered by the regex list (4 architect files via `^packages/architect/`, 3 core files as explicit literals).
...
describe('runWorkItemCycle', () => {
  it('happy path — plan ok + build ok → cycle outcome=built, work item awaiting_human', async () => {
    ...
    expect(result.outcome).toBe('built') // PR creation comes in Plan 2
    ...
    // Plan 1 leaves the work item awaiting human pickup — Plan 2 progresses to 'merged'.
...
export async function runWorkItemCycle(deps: RunCycleDeps): Promise<RunCycleOutcome> {
  ...
  while (iteration <= workItem.budgetIters) {
    ...
    // and transitions the cycle from 'built' → 'merged' on PR merge.
    cycleStore.terminate(cycle.id, {
      outcome: 'built',
      planText: plan.plan,
      diffText: plan.diff,
    ...
    // Mirror to the work item — Plan 1 leaves the work item in 'awaiting_human'
    // so the operator can pick up the branch. Plan 2 will progress this to
    // 'merged' automatically when the PR closes green.
    workStore.updateStatus(workItem.id, 'awaiting_human')
...
describe('ArchitectDaemon end-to-end', () => {
  it('processes a pending work item through plan + build with mocked deps', async () => {
    ...
    expect(pendingAfter).toHaveLength(0) // no longer pending
    // Item moved to awaiting_human (Plan 1 terminal state); cycle outcome is 'built'.
