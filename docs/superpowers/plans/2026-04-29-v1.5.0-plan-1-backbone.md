...
Add a one-line throwaway to confirm imports work end-to-end before any real code lands:
...
describe('workspace link', () => {
  it('can import from @coastal-ai/core (root)', async () => {
    // The bare specifier should resolve. We don't care what's exported —
    // only that the resolution chain works.
...
Run: `pnpm --filter architect build && pnpm --filter core build && pnpm --filter architect exec vitest run src/__tests__/workspace-link.test.ts`
Expected: PASS (or at minimum, no `ERR_MODULE_NOT_FOUND`).

> ⚠️ **If the test fails:** dev workflows that don't run `pnpm build` first won't see the dist outputs. Check whether the project uses TypeScript Project References (`references` in tsconfig) or relies on `ts-node` resolving `.ts` directly. If neither, add an explicit `pnpm --filter core build` step to the architect's `pretest` script in `packages/architect/package.json`.
...
Read existing `packages/architect/src/__tests__/config.test.ts` first. The current file likely asserts a smaller / older default list; **delete or update any assertions that contradict the new defaults below** rather than leaving conflicting expectations side-by-side. Specifically:

1. Check current behavior of `isLockedPath(path)` — does it return `null` on allow (and a string on deny), or `false`/`undefined`? The new tests below expect `null`. Update existing assertions if they assume a different return type.
2. Check if existing tests assert exact regex-array contents (e.g., `expect(DEFAULT_LOCKED_PATHS).toHaveLength(N)`). Those will break with the expanded list — delete those assertions; the behavior tests below subsume them.
3. Check if any existing test expects `packages/architect/**` to be allowed. That assertion is now wrong (self-modify guard) — delete it.
