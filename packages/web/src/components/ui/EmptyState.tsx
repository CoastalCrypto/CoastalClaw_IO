...
export function EmptyState({ icon, title, description, action }: Props) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-xs mb-5 max-w-xs leading-relaxed" style={{ color: '#A0AEC0' }}>{description}</p>
      {action && (
        <button
          onClick={action.onClick}
          className="text-xs font-mono px-4 py-2 rounded-lg transition-colors"
          style={{ background: 'rgba(0,212,255,0.12)', border: '1px solid rgba(0,212,255,0.30)', color: '#00D4FF', cursor: 'pointer' }}
        >
          {action.label}
        </button>
      )}
